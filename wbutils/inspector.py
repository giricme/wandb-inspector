...
from .discovery import MetricsDiscovery, MetricsRegistry, MetricSource
from .dedup import RunDeduplicator, DedupConfig, DedupStrategy
from .report import ReportGenerator, LaTeXFormatter, AggregatedReport
...
class WandbInspector:
    ...
    def fetch_runs(
        ...
        for run in wandb_runs:
            try:
                ...
                print(f"Warning: Recursion error caching run {getattr(run, 'id', 'unknown')}, skipping")
                continue
    ...
    def discover_metrics(
        ...
        if scan_history and not refresh and verbose:
            print("Note: History scanning skipped (using cached data). Use refresh=True for history metrics.")
