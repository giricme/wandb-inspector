...
def apply_filters(runs: list, filters: dict[str, list[str]], show_debug_on_empty: bool = True) -> list:
    """
    ...
    if not filtered and show_debug_on_empty and runs:
        ...
        for key, filter_values in filters.items():
            ...
            if missing_count == len(runs):
                ...
                print(f"    actual values: {actual_list}" + (" ..." if len(actual_values) > 10 else ""))
                # Check for near matches
                for fv in filter_values:
                    near = [av for av in actual_values if av and fv.lower() == av.lower() and fv != av]
                    if near:
...
def parse_metric_maps(metric_map_args: list[str] | None) -> dict[str, dict[str, str]]:
    ...
    for m in metric_map_args:
        if ":" in m and "=" in m:
            ...
            print(f"Warning: Invalid metric-map format '{m}' (expected LABEL=metric or LABEL:canonical=actual)")
    
    return mappings


def get_mapped_metric(label: str, canonical_metric: str, metric_maps: dict, metrics: list[str]) -> str:
...
def parse_project_specs(project_args: list[str] | None, default_entity: str | None) -> list[tuple[str, str, str]]:
    """
...
Examples:
  ...
  wbutils export my-entity my-project --metrics accuracy --output results.csv
    ...
    def add_analysis_args(p):
        ...
        p.add_argument("entity", nargs="?", default=None, 
        ...
        p.add_argument("--project", dest="projects", action="append", metavar="LABEL:PROJECT",
    ...
    discover_parser.add_argument("--max-runs", type=int, default=50, help="Max runs to scan per project")
    ...
    metrics_parser.add_argument("--source", choices=["summary", "history", "both"], help="Filter by source")
    ...
    configs_parser.add_argument("--min-runs", type=int, default=1, help="Min runs that have this key")
    configs_parser.add_argument("--pattern", help="Filter by name pattern")
    configs_parser.add_argument("--show-values", action="store_true", help="Show unique values for each key")
    ...
    inspect_parser.add_argument("--max-runs", type=int, default=10, help="Max runs to inspect for history")
    ...
    report_parser.add_argument("--group-by", nargs="+", help="Config key(s) to group by (e.g., --group-by env_name seed)")
    report_parser.add_argument("--no-std", action="store_true", help="Hide standard deviation")
    report_parser.add_argument("--format", choices=["latex", "text"], default="text", help="Output format")
    ...
    history_parser = subparsers.add_parser("history", help="Fetch and cache metric history (time series)")
    add_common_args(history_parser)
    history_parser.add_argument("--metric", required=True, help="Metric name to fetch")
    history_parser.add_argument("--config-keys", nargs="+", help="Config keys to include when exporting")
    ...
    history_parser.add_argument("--step-key", default="_step", help="Step column name (default: _step)")
    history_parser.add_argument("--refresh", action="store_true", help="Force re-fetch from wandb (ignore cache)")
    ...
    plot_parser.add_argument("entity", nargs="?", default=None, 
                            help="Default entity (optional if using entity/project in --project)")
    plot_parser.add_argument("project", nargs="?", default=None, 
                            help="Wandb project name (optional if using --project)")
    plot_parser.add_argument("--project", dest="projects", action="append", metavar="LABEL:PROJECT",
                            help="Project with label. Formats: 'label:project' (uses default entity), "
                                 "'label:entity/project' (explicit entity). Can be repeated.")
    plot_parser.add_argument("--cache-dir", help="Directory to cache data (default: .cache/wbutils)")
    plot_parser.add_argument("--no-cache", action="store_true", help="Disable caching")
    plot_parser.add_argument("--filter", action="append", dest="filters", metavar="KEY=VALUE",
                            help="Filter runs by config (e.g., --filter env_name=square). Repeat for AND.")
    plot_parser.add_argument("--dedup-keys", nargs="+", default=None, help="Config keys for deduplication")
    plot_parser.add_argument("--dedup-strategy", choices=["latest", "longest", "best"], default="latest",
                            help="Strategy for selecting among duplicates")
    ...
    plot_parser.add_argument("--metric-map", action="append", dest="metric_maps", metavar="LABEL=METRIC",
    ...
    plot_parser.add_argument("--std-bands", action="store_true", default=None, help="Show std deviation bands")
    plot_parser.add_argument("--no-std-bands", action="store_true", help="Disable std deviation bands")
    plot_parser.add_argument("--smooth", type=float, default=0, metavar="ALPHA",
                            help="Smoothing factor (0-1). 0=none, 0.9=heavy smoothing (EMA)")
    plot_parser.add_argument("--no-align-zero", action="store_true", 
    ...
    plot_parser.add_argument("--figsize", nargs=2, type=float, default=[10, 6], help="Figure size (width height)")
...
def cmd_plot(args, strategy_map):
    ...
    for label, inspector in inspectors.items():
        for metric in metrics:
            ...
            if not inspector.has_history(actual_metric):
                ...
                print(f"Run first: wbutils history {inspector.entity} {inspector.project} --metric {actual_metric}")
                return
    ...
    if len(inspectors) > 1:
        for metric in metrics:
            ...
            if len(unique_step_keys) > 1:
                ...
                for label, inspector in inspectors.items():
                    actual_metric = get_mapped_metric(label, metric, metric_maps, metrics)
                    print(f"  wbutils history {inspector.entity} {inspector.project} --metric {actual_metric} --step-key _step --refresh")
                return
    ...
    if filters and not all_history:
        if total_runs_after_filter == 0:
            ...
            print(f"Warning: {total_runs_after_filter} runs matched filters, but no history data found.")
            ...
            for label, inspector in inspectors.items():
                for metric in metrics:
                    ...
                    print(f"  {label}: {len(history_run_ids)} history runs, {len(filtered_run_ids)} filtered runs, {len(overlap)} overlap")
        return
    ...
    for split_idx, split_val in enumerate(split_values):
        ...
        for metric_idx, metric in enumerate(metrics):
            ...
            has_multiple = any(
                any(len(vals) > 1 for vals in steps.values())
            ...
            for group_key in sorted(grouped.keys(), key=str):
                ...
                if smooth_alpha > 0 and len(means) > 1:
                    ...
                    for i in range(1, len(means)):
                        smoothed_means[i] = smooth_alpha * smoothed_means[i-1] + (1 - smooth_alpha) * means[i]
                    ...
                    if np.any(stds > 0):
                        ...
                        for i in range(1, len(stds)):
                            smoothed_stds[i] = smooth_alpha * smoothed_stds[i-1] + (1 - smooth_alpha) * stds[i]
                        stds = smoothed_stds
